...
class MatchPlugin final : public BasePlugin<AlgorithmT, FacadeT>
{
  public:
  ...
  private:
    ...
    mutable routing_algorithms::MapMatching<AlgorithmT, FacadeT> map_matching;
...
template <typename AlgorithmT, template <typename A> class FacadeT>
Status
MatchPlugin<AlgorithmT, FacadeT>::HandleRequest(const std::shared_ptr<FacadeT<AlgorithmT>> facade,
                                                const api::MatchParameters &parameters,
                                                util::json::Object &json_result) const
{
