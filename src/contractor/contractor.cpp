                  ...
                  EdgeWeight &new_segment_duration)
{
    ...
    new_segment_duration = (speed_iter->speed_source.speed > 0)
        ...
        std::for_each(
            ...
            [&](const std::string &line) {
                ...
                TurnPenaltySource val{
                    ...
                    {duration, weight, static_cast<std::uint8_t>(file_id)}};
                local.push_back(std::move(val));
            });
...
#pragma pack(pop, r1)
    ...
    if (update_edge_weights)
    {
        ...
        geometry_datasource.resize(geometry_fwd_duration_list.size(), 0);
        ...
        tbb::parallel_for_each(first, last, [&](const LeafNode &current_node) {
            ...
            for (size_t i = 0; i < current_node.object_count; i++)
            {
                ...
                const auto current_fwd_duration =
                    geometry_fwd_duration_list[forward_begin + leaf_object.fwd_segment_position];
                ...
                if (forward_speed_iter != segment_speed_lookup.end())
                {
                    ...
                    geometry_fwd_weight_list[forward_begin + 1 + leaf_object.fwd_segment_position] =
                        new_segment_weight;
                    geometry_fwd_duration_list[forward_begin + 1 +
                ...
                const auto current_rev_duration =
                    geometry_rev_duration_list[forward_begin + leaf_object.fwd_segment_position];
    ...
    auto edge_based_edge_last = reinterpret_cast<const extractor::EdgeBasedEdge *>(
        reinterpret_cast<char *>(edge_based_graph_region.get_address()) +
        sizeof(EdgeBasedGraphHeader) +
        sizeof(extractor::EdgeBasedEdge) * graph_header.number_of_edges);

    // Mapped file pointer for edge-based graph segments
            ...
            if (update_turn_penalties)
            {
                ...
                const auto turn_iter =
                    find(turn_penalty_lookup,
                         TurnPenaltySource{
                             {turn_index.from_id, turn_index.via_id, turn_index.to_id}, {0, 0, 0}});
